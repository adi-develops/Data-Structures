...
void delete_begin(node *&start, node *&end){
    node *temp=start;
    if(temp==NULL){
        cout<<"Empty list.";
        return;
    }
    else if(start==end){  //start->next==NULL
        start=end=NULL;
        delete temp;
    }
    else{
...
void delete_loc(node *&start, node *&end, int loc){
    ...
    if(loc==1){
        ...
        for(int i=1; i<=loc-2; i++){
            temp=temp->next;
            if(temp->next==NULL){
                cout<<"Location greater than number of elements.";
                return;
            }
        }
        temp1=temp->next;
...
int main(){
    ...
    cout<<endl;
    delete_begin(start, end);
    delete_end(start, end);
    delete_loc(start, end, 9);
    traverse_start(start);
    cout<<endl<<end->info;
    return 0;
}